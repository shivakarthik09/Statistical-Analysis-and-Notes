...
print(value_counts)
# Create a bar plot
barplot(value_counts, main = "Number of people staying in there house", xlab = "No of people", ylab = "Number of families", col = "skyblue")
# Class
value_counts <- table(gss$class)
print(value_counts)

# Example 1

#The claim is that the number of hours worked per week is, on average, 40 hours. 
...
library(infer) # Make sure to install this package
...
z = (phat - 0.4)/(sqrt(0.4*(1-0.4)/n))   # standard error of sample proportion
z

pnorm(z) #for getting P value
